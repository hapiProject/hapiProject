/*
 * file: patients.js
 * project: login_nodejs
 * author: eoin 17252409
 * email: eoingoslin@gmail.com
 * created: Tuesday, 24th April 2018 2:09:34 pm
 * modified: Tuesday, 24th April 2018 6:01:41 pm
 * filepath: /run/user/1000/gvfs/sftp:host=139.59.177.196,user=root/root/NewestHAPI/hapi/public/JS/patients.js
 * comment: comment
 */
//
  var config = {
  ...
  */
  ...
  function personChange (userinput) { //changes the person which references everything else
    ...
    rootRef.on('value',snap => {
        ...
        objectId.innerHTML = snap.val().UserId;
...
function addPatientButtons(number){ // dynamically makes the new buttons based on how many patients
   var div = document.getElementById('newStuff');
          ...
          but.value=number;
          but.setAttribute("id", "basestyle");
          but.setAttribute("class", "animated bounceInLeft hvr-underline-from-center");
}

function addPatientDay(day){  //dynamically adds the days of the selected patient
          var div = document.getElementById('daysDiv');
          var but = document.createElement("button");
          but.setAttribute("id", "daystyle");
          but.setAttribute("class", "hvr-shutter-in-vertical");
...
function styleRecords() {
  //document.getElementById('pat').style.textDecoration = "underline";
  //setTimeout('$(#pat).css("text-decoration", "underline")',4000);
  setTimeout('$("#pat").css("text-decoration", "underline")', 1000);
}
